...
#################### Download Data ##################
Music.Pure <- get_artist_audio_features('dr. dre', include_groups = "appears_on"); Music.1 <- Music.Pure
...
#################### Code Dre Songs ##################
Music.3$Feature <- "No"
Music.3$Feature[grepl( "Dr. Dre" , Music.3$track_name ) == T] <- "Yes"
...
dre.net <- network(Music.4, directed = FALSE)

dre.net  %v% "Artist" <- Music.4$Album_Artists

head(unique(ggnetwork(dre.net)))

ggnetwork(dre.net)$Artist


ggplot(dre.net, aes(x = x, y = y, xend = xend, yend = yend)) +
  geom_edges(color = "black") +
  geom_nodelabel_repel(aes(label = Artist[ vertex.names ]),
                       fontface = "bold", box.padding = unit(1, "lines")) +
  geom_nodes(color = "black", size = 2) +
  theme_blank()

set.seed(10052016)
ggnet2(mm.net,labelon = TRUE, label.color = "black",
